...
- **Artefacts-Sektion im Item.** Jedes featurework-Item trägt eine `## Artefacts`-Sektion zwischen day-zero framing und der ersten Stage-Sektion. Sie indiziert die Outputs per Stage: `pending` beim Anlegen, wird beim jeweiligen Stage-Outcome-Write mit einem Markdown-Link auf das Artefakt (oder `not applicable — <reason>`) gebumpt. Bump ridet den Stage-Approval-Commit — kein separater. Optional für bug/question. Details in `workflow-backlog` SoT.
