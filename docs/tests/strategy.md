...
> **Template file.** The Work-item anchoring section and the Verification-modes section are canonical (workflow-level SoT — do NOT rewrite them per project). The Behavioral / Structural / Procedural framing sections describe universal conventions that apply everywhere. Only the "CI Integration" section at the bottom is expected to be filled in per project with the concrete test-runner commands for your tech stack. Drop this banner and the frontmatter once the CI Integration section is filled in.
...
<Fill in per project. Name each test runner and which mode it covers.>

- Behavioral (<language X>) — `<test command>`, blocking merge on failure.
- Behavioral (<language Y>) — `<test command>`, blocking merge on failure.
- Full stack — `<project-owned runner, e.g. python scripts/test.py>`, invoked by the orchestrator.
- Structural — invoked from the same runner OR from a dedicated `scripts/verify-structure.sh`; blocking merge on failure.
- Procedural — never CI. Verified by humans as part of PR review or manual acceptance; results recorded in the test file's `**Last verified:**` field.
