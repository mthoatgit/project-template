...
> **Current focus (Stand 2026-07-19, für nächste Session):** Wiedereinstieg bei Item [028 · DoD-Checkpoints + Struktur-Muster + Umsetzungsplan „Option H"](topics/028-definition-of-done-for-work-items.md) — **explizit vom Nutzer als das aktuell wichtigste Thema markiert.** Session-Marathon 2026-07-18/19 hat den Loop-Flow durch mehrere Iterationen (E → F → G → H) auf eine sequenzielle Gate-Pipeline mit Design-Rückkehr konvergiert: `ralph → struktur_check → docs_write → final_approval (3-way)`. Vier Guardrails gegen Reviewer-Fehlklassifikation, Structured Output mit Kriterium-Enum, Cost-Tabelle inklusive Design-durch-Docs. Item enthält am Ende „Was noch offen ist"-Block mit sieben konkreten Umsetzungs-Fragen (Verankerungsort, Mandatory-Files-Config, `orchestrator/claude.py` Caching-Sichtprüfung, `MAX_DOCS_CYCLES`, Feedback-Routing, Interaktion mit Struktur-Mustern, Debug-Logs). **Erster mechanischer Schritt beim Session-Start:** 5-Min-Sichtprüfung in `orchestrator/claude.py` ob explizites Prompt-Caching via `cache_control`-Marker läuft oder nur automatisch. **Wenn du dieses Item startest, entferne diese Callout-Zeile** — sie ist Session-Handoff, nicht Dauer-Zustand. Vorheriger Focus (022) bleibt offen im Backlog, aber depriorisiert.
