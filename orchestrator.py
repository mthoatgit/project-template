...
#          specific files. Every project generated from this template
#          ships a `scripts/test.py` runner (written by /scaffold from
#          the tech stack in system-design.md) that dispatches to the
#          right test tools — pytest, flutter test, mvn, gradle, etc.
#  REQ-40  --test-cmd defaults to 'python scripts/test.py'. If the
#          default is in use AND scripts/test.py does not exist, the
#          orchestrator exits with code 1 and a message pointing at
#          /scaffold. Override with an explicit --test-cmd for projects
...
def _run(args, tasks, project_dir: str, log_path) -> None:
    ...
    # ── Prerequisite check: default test runner exists ────────
